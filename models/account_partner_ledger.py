...
class ReportPartnerLedger(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if data['form']['initial_balance'] and data['form']['partner_ids'] :
            self.env.cr.execute(
                """select distinct a.partner_id,b.name from account_move_line a join res_partner b on a.partner_id=b.id where partner_id is not null order by name""")
            ...
            new_query = ''
            add_query = f"""SELECT  c.name, sum(a.debit) as initial_debit, sum(a.credit) as initial_credit
            from account_move_line a join account_move b on a.move_id = b.id join res_partner c on a.partner_id = c.id  where a.partner_id is not null 
            """
